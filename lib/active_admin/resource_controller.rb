  ...
  class ResourceController < ::InheritedViews::Base
    ...
    action_item :only => :show do
      if controller.action_methods.include?('edit')
        link_to(t('admin.actions.edit', :name => active_admin_config.resource_name), edit_resource_path(resource))
      end
    end

    action_item :only => :show do
      if controller.action_methods.include?("destroy")
        link_to(t('admin.actions.delete', :name => active_admin_config.resource_name),
    ...
    action_item :except => [:new, :show] do
      if controller.action_methods.include?('new')
        link_to(t('admin.actions.new', :name => active_admin_config.resource_name), new_resource_path)
      end
    end
